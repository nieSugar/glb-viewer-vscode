...
From version 3.0.0 with the launch of the [web version](https://niesugar.github.io/glb-viewer-web/) all core files are now in a separate repository: https://github.com/ohzinteractive/glb-viewer-core. This repository is now only a wrapper for the core files with some VS Code specific code.
...
- Edit selected-object details directly: press Enter or leave a field to apply, Esc to reset unfinished input, or Undo last edit to revert the latest change on the selected object. Visibility is shown by default and toggles immediately; JSON also supports Ctrl/Cmd+Enter. Use Details Settings to show additional fields. Changes affect the current preview only and are not saved to the source GLB/GLTF file. Transform edits stop animations; instanced meshes are edited as a whole.
